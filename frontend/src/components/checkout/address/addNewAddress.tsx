...
export default function AddNewAddress({ onAdd, onCancel }: Props) {
  const [form, setForm] = useState<Omit<Address, 'id'>>({ // Use Omit<Address, 'id'> for form state
  ...
  function handleChange(e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) { // Added TextAreaElement
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    } as Address; // Type assertion as crypto.randomUUID() is not in global namespace in some environments

    try {
      const addresses = getSavedAddresses(); // Assuming this returns Address[]
      const updated = [...addresses, newAddress];
      saveAddresses(updated); // Assuming this saves Address[]
      onAdd(newAddress); // Callback to parent
  ...
  const inputClasses = "w-full p-3 rounded-md bg-zinc-900 border border-gray-700 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-cyan-500 transition-all duration-200";
  ...
  return (
    <Glass className="p-6 sm:p-8 space-y-6">
      <h2 className="text-2xl sm:text-3xl font-bold text-white text-center mb-6">Add New Address</h2>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label htmlFor="addressLine2" className={labelClasses}>Address Line 2 (Optional)</label>
          <input
            type="text"
            id="addressLine2"
            name="addressLine2"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            <label htmlFor="pincode" className={labelClasses}>Zip / Postal Code</label>
            <input
          ...
          <div className="col-span-full sm:col-span-1 lg:col-span-1"> {/* Ensure proper spanning */}
